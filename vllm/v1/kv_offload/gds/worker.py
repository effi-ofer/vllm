...
class GDSOffloadingWorker(OffloadingWorker):
    ...
    def __init__(
        ...
        logger.info(
            ...
            len(self._gpu_tensors),
            nbytes / (1 << 20) if self._gpu_tensors else 0,
        )
    ...
    def _do_batch_io(
        ...
        """Execute batch cuFile I/O for one file."""
        n = len(ops)
            ...
            while completed < n:
                ...
                for j in range(got):
                    if events[j].status == CUFILE_FAILED:
                        raise RuntimeError(f"cuFileBatchIO failed: ret={events[j].ret}")
                completed += got
        finally:
