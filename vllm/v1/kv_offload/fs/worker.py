...
class FSOffloadingWorker(OffloadingWorker):
    ...
    def write_block(
        ...
        Returns:
            Optional context object (e.g. file handles) to be stored
            in the _Transfer and cleaned up in on_transfer_done().
        """
    ...
    def read_block(
        ...
        Returns:
            Optional context object for cleanup.
        """

    def on_transfer_done(self, transfer: _Transfer) -> None:
        """Called when all futures for a transfer complete.

        Subclasses can override to clean up per-transfer resources
        (e.g. deregister file handles).
        """

    def init_backend(self) -> None:
        """Optional one-time backend initialization (e.g. cuFileDriverOpen)."""
