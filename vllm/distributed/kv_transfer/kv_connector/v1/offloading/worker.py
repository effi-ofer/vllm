...
class OffloadingConnectorWorker:
    ...
    def __init__(self, spec: OffloadingSpec):
        ...
        self._unsubmitted_store_jobs: list[
            tuple[int, DeviceLoadStoreSpec, LoadStoreSpec]
        ] = []
    ...
    def _resolve_device_ptrs(self, device_spec: DeviceLoadStoreSpec) -> DevicePointers:
        """Resolve logical block IDs to device memory pointers."""
    ...
    def _submit_pending_stores(self) -> None:
        """Resolve and submit all deferred store jobs."""
        assert self.worker is not None
        for job_id, device_spec, medium_spec in self._unsubmitted_store_jobs:
            device_ptrs = self._resolve_device_ptrs(device_spec)
            success = self.worker.submit_store(job_id, device_ptrs, medium_spec)
            assert success
    ...
    def start_kv_transfers(self, metadata: OffloadingConnectorMetadata):
        ...
        for job_id, entry in metadata.load_jobs.items():
            ...
            assert isinstance(entry.dst_spec, DeviceLoadStoreSpec)
    ...
    def prepare_store_kv(self, metadata: OffloadingConnectorMetadata):
        for job_id, entry in metadata.store_jobs.items():
            ...
            assert isinstance(entry.src_spec, DeviceLoadStoreSpec)
